...
//	private final SensorDao sensorDao = new SensorDao(null);
   ...
   // 	List<Service> services = serviceDao.getServices();
        		...
        		if(location.getSensors() != null) {
	        		if(location.getSensors().size() == 0) {
	    				...
	    				for(Sensor sensor : location.getSensors()) {
	    					...
	    					rowData [i][2] = String.valueOf(sensor.getId());
	...
	public JPanel createSensorsPanel(){
		...
		final String sensorId = (String) model.getValueAt(table.getSelectedRow(), 2);
		...
		if(sensorId != null) {
			sensorCodeField.setText(""+sensorId);
		} else {
			...
			public void actionPerformed(ActionEvent event) {
				for(Sensor sensor : sensors) {
					if(sensor.getId()==Integer.parseInt(sensorId)) {
						sensor.setId(Integer.parseInt(sensorCodeField.getText()));
